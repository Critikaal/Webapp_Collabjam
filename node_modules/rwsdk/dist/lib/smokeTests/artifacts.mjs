...
export async function setupArtifactsDirectory(artifactDir, options) {
    ...
    if (exists) {
        ...
        try {
            ...
            console.error(`⚠️ Could not remove existing artifacts directory: ${error instanceof Error ? error.message : String(error)}`);
    ...
    try {
        ...
        console.error(`⚠️ Warning: Log directory permissions issue: ${error instanceof Error ? error.message : String(error)}`);
