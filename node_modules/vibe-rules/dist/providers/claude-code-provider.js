...
export class ClaudeCodeRuleProvider {
    constructor() {
        this.ruleType = RuleType.CLAUDE_CODE;
    }
    /**
     ...
     */
    async appendRule(name, targetPath, isGlobal = false, options) {
     ...
     */
    async appendFormattedRule(config, targetPath, isGlobal, options) {
        ...
        if (match) {
            ...
            console.log(chalk.blue(`Updating existing rule block for "${config.name}" in ${destinationPath}...`));
            ...
            console.log(chalk.blue(`Appending new rule block for "${config.name}" to ${destinationPath}...`));
