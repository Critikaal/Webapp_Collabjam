 ...
 */
function displayAppliedMetadata(metadata) {
 ...
 */
export async function loadCommandAction(name, editor, options) {
    try {
        ...
        const ruleType = editor.toLowerCase();
        ...
        const success = await provider.appendFormattedRule(ruleConfig, finalTargetPath, options.global, generatorOptions);
        if (success) {
            console.log(chalk.green(`Rule "${name}" applied successfully for ${editor} at ${finalTargetPath}`));
        ...
        console.error(chalk.red(`Error loading rule: ${error instanceof Error ? error.message : error}`));
        process.exit(1);
    }
}
