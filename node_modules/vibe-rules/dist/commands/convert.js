 ...
 */
export async function convertCommandAction(sourceFormat, targetFormat, sourcePath, options) {
    ...
    try {
        ...
        try {
            ...
            debugLog(`Source stats: isDirectory=${sourceStats.isDirectory()}, isFile=${sourceStats.isFile()}`);
        }
        catch (error) {
        ...
        try {
            ...
            console.error(chalk.red(`❌ Error extracting rules: ${error instanceof Error ? error.message : String(error)}`));
            process.exit(1);
        }
        if (rules.length === 0) {
        ...
        for (const rule of rules) {
            try {
                ...
                const success = await targetProvider.appendFormattedRule(ruleConfig, finalTargetPath, options.global, rule.metadata);
                if (success) {
                    successCount++;
                ...
                console.error(chalk.red(`❌ Error converting rule "${rule.name}": ${error instanceof Error ? error.message : String(error)}`));
            }
        }
        console.log(chalk.green(`\n🎉 Successfully converted ${successCount}/${rules.length} rules from ${sourceFormat} to ${targetFormat}`));
    }
    catch (error) {
        console.error(chalk.red(`❌ Convert command failed: ${error instanceof Error ? error.message : String(error)}`));
        process.exit(1);
    }
}
/**
 ...
 */
async function extractRulesFromDirectory(sourceType, dirPath) {
    switch (sourceType) {
 ...
 */
async function extractRulesFromFile(sourceType, filePath) {
    switch (sourceType) {
        case RuleType.CURSOR:
 ...
 */
async function extractFromCursorDirectory(dirPath) {
    const rules = [];
 ...
 */
async function extractFromCursorFile(filePath) {
    ...
    if (parsed.frontmatter.globs)
        metadata.globs = parsed.frontmatter.globs;
    return [
        {
            name: ruleName,
 ...
 */
async function extractFromClinerulesDireatory(dirPath) {
    const rules = [];
 ...
 */
async function extractFromVSCodeDirectory(dirPath) {
    const rules = [];
    if (!(await fsExtra.pathExists(dirPath))) {
 ...
 */
async function extractFromVSCodeFile(filePath) {
    const content = await fs.readFile(filePath, "utf-8");
    const parsed = parseFrontmatter(content);
 ...
 */
async function extractFromTaggedFileWithWrapper(filePath, wrapperStart) {
    const content = await fs.readFile(filePath, "utf-8");
    const rules = [];
    // Find the wrapper block
    const wrapperEnd = wrapperStart.replace("vibe-rules Integration", "/vibe-rules Integration");
    const wrapperRegex = new RegExp(`${escapeRegex(wrapperStart)}([\\s\\S]*?)${escapeRegex(wrapperEnd)}`);
 ...
 */
function removeMetadataFromContent(content) {
    const lines = content.split("\n");
 ...
 */
function validateAndGetRuleType(format, type) {
    ...
    if (!ruleType) {
        ...
        console.error(chalk.red(`❌ Invalid ${type} format: ${format}. Available formats: ${available}`));
        return null;
    }
    return ruleType;
}
 ...
 */
function isMultiFileProvider(ruleType) {
    switch (ruleType) {
        case RuleType.CURSOR:
        case RuleType.CLINERULES:
        case RuleType.ROO:
